...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    const baseClasses = `
      w-full p-4 rounded-xl bg-white/10 text-gray-900 placeholder-transparent 
    ...
    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(99,102,241,0.4)] border-[#ef4444]"
      : "border-white/20 hover:border-[#ef4444]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-600 transition-colors group-hover:text-[#ef4444]">
        <Icon className="w-5 h-5" />
        <label
          htmlFor={field}
          className={`
            absolute left-12 top-1/2 transform -translate-y-1/2 text-gray-600 text-sm transition-all duration-300 
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[#ef4444] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[#ef4444]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
